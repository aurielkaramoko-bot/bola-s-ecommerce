...
public SecurityFilterChain securityFilterChain(HttpSecurity http) throws Exception {
    ...
    http
        ...
        .headers(headers -> {
            ...
            headers.contentSecurityPolicy(csp -> csp.policyDirectives(
                "default-src 'self'; img-src 'self' data: https:; media-src 'self' https://res.cloudinary.com; "
                + "script-src 'self' 'unsafe-inline' https://cdn.jsdelivr.net https://maps.googleapis.com; "
                + "style-src 'self' 'unsafe-inline' https://cdn.jsdelivr.net https://fonts.googleapis.com; "
                + "font-src 'self' https://cdn.jsdelivr.net https://fonts.gstatic.com; "
                + "connect-src 'self' https://maps.googleapis.com; frame-ancestors 'none'; object-src 'none';"));
    ...
    http.authorizeHttpRequests(auth -> auth
        .requestMatchers(
            ...
            "/customer/oauth2/success", "/vendor/login", "/vendor/register",
            "/boutiques", "/boutiques/**", "/error"
        ).permitAll()
        .requestMatchers("/h2-console/**").access((auth, ctx) -> 
    ...
    // OAuth2 Google → Désactivé temporairement sur Render
    // http.oauth2Login(...) 
