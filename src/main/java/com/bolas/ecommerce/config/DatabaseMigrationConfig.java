    ...
    CommandLineRunner migrateDatabase(DataSource dataSource, Environment environment) {
        return args -> {
            ...
            try (Connection conn = dataSource.getConnection()) {
                ...
                // Migration 37 : Table interactions client-produit (moteur de recommandation IA)
                try {
                    conn.createStatement().execute("""
                    ...
                    """);
                    conn.createStatement().execute(
                        "CREATE INDEX IF NOT EXISTS idx_cpi_customer ON customer_product_interactions(customer_id)");
                    conn.createStatement().execute(
                        "CREATE INDEX IF NOT EXISTS idx_cpi_product ON customer_product_interactions(product_id)");
                    conn.createStatement().execute(
                        "CREATE INDEX IF NOT EXISTS idx_cpi_customer_type ON customer_product_interactions(customer_id, interaction_type)");
                    conn.createStatement().execute(
                        "CREATE INDEX IF NOT EXISTS idx_cpi_created ON customer_product_interactions(created_at)");
                    log.info("Migration 37: table customer_product_interactions créée");
                } catch (SQLException e) {
                    log.warn("Migration 37 customer_product_interactions: {}", e.getMessage());
                }

                // Migration 38 : Table TrustScore vendeur (détection de fraude)
                try {
                    conn.createStatement().execute("""
                        CREATE TABLE IF NOT EXISTS vendor_trust_scores (
                            ...
                            delivery_points INT NOT NULL DEFAULT 0,
                            seniority_points INT NOT NULL DEFAULT 0,
                            review_points INT NOT NULL DEFAULT 0,
                            response_points INT NOT NULL DEFAULT 0,
                            profile_points INT NOT NULL DEFAULT 0,
                            volume_points INT NOT NULL DEFAULT 0,
                            report_penalty INT NOT NULL DEFAULT 0,
                            trust_level VARCHAR(20) NOT NULL DEFAULT 'NEW',
                            last_calculated_at TIMESTAMP WITH TIME ZONE,
                            flagged_for_review BOOLEAN NOT NULL DEFAULT FALSE,
                            flag_reason VARCHAR(500),
                            CONSTRAINT fk_vts_vendor FOREIGN KEY (vendor_id) REFERENCES vendor_users(id) ON DELETE CASCADE
                        )
                    """);
                    log.info("Migration 38: table vendor_trust_scores créée");
                } catch (SQLException e) {
                    log.warn("Migration 38 vendor_trust_scores: {}", e.getMessage());
                }
